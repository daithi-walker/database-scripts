...
DB=$1
LOGFILE=/var/tmp/bloat_report_${DB}.txt
SERVER=`hostname`

echo "Bloat report for {$DB}" >$LOGFILE
echo >>$LOGFILE
echo "Index bloat: " >>$LOGFILE
...
SELECT 'TOTAL:', SUM(ituples), sum(ipages), sum(iotta), sum(ibloat), sum(wastedipages), sum(wastedibytes), pg_size_pretty(sum(wastedibytes)::bigint) FROM x;
" >> $LOGFILE

echo "Table bloat: " >>$LOGFILE
psql -U postgres -d ${DB} -c "
WITH x AS (
SELECT
...
SELECT * FROM x UNION ALL
SELECT 'TOTAL:', SUM(reltuples), sum(relpages), sum(otta), sum(tbloat), sum(wastedpages), sum(wastedbytes), pg_size_pretty(sum(wastedbytes)::bigint) FROM x;
