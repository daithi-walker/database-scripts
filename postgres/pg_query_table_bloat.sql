...
WITH x AS
(
SELECT  CASE WHEN sml.relpages < sml.otta THEN 0 ELSE sml.bs*(sml.relpages-sml.otta)::bigint END AS wastedbytes
FROM    (
        SELECT  cc.relpages
        ,       rs.bs
...
SELECT  SUM(dummy.wastedbytes) AS wastedbytes
FROM    (SELECT x.wastedbytes FROM x UNION ALL SELECT 0 AS wastedbytes) dummy
