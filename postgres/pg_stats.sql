...
with table_stats as
    ...
    ,       psut.relname
    ,       psut.n_live_tup
    ,       round(1.0 * psut.idx_scan / greatest(1, psut.seq_scan + psut.idx_scan),2) as index_use_ratio
...
,   table_io as
    ...
    ,       round(sum(psiut.heap_blks_hit) / greatest(1, sum(psiut.heap_blks_hit) + sum(psiut.heap_blks_read)),2) as table_hit_ratio
...
,   index_io as
    ...
    ,       round(1.0 * sum(psiui.idx_blks_hit) / greatest(1.0, sum(psiui.idx_blks_hit) + sum(psiui.idx_blks_read)),2) as idx_hit_ratio
    from    pg_statio_user_indexes psiui
    group by
            psiui.relname
    ,       psiui.indexrelname
    order by
            sum(psiui.idx_blks_read) desc
    )
